...
(function(a, b) {
    ...
    var a = {
      ...
      , b = function(a, b, c, d, e, f, g) {
        switch (this.pushed = !1,
        this.element = document.createElement("div"),
        this.element.className = c.classes || "",
        ...
        this.element.style.borderColor = c.borderColor,
        this.element.style.boxSizing = "border-box",
        this.element.style.backgroundColor = c.backgroundColor,
        "undefined" != typeof c.icon) {
            ...
            h.innerHTML = c.icon,
            -1 == c.icon.indexOf("<img") ? -1 != c.icon.indexOf("<svg ") && (h.childNodes[0].style.width = "16px",
            h.childNodes[0].style.height = "16px",
            "undefined" != typeof c.iconColor && (h.childNodes[0].style.fill = c.iconColor)) : (h.childNodes[0].style.width = "16px",
            h.childNodes[0].style.height = "16px"),
            "undefined" != typeof c.iconClasses && (h.childNodes[0].className += c.iconClasses),
            this.element.appendChild(h)
        }
        var i = document.createElement("span");
        i.style.color = c.textColor,
        i.innerHTML = b,
        this.element.appendChild(i);
        var j = document.createElement("p");
        switch (j.className = "progress",
        ...
        j.style.height = "2px",
        j.style.content = " ",
        j.style.backgroundColor = c.progressColor,
        j.style.marginBottom = 0,
        this.element.appendChild(j),
        a.options.direction) {
        case "top":
            a.container.insertBefore(this.element, a.container.childNodes[0]);
            break;
        case "bottom":
        default:
            a.container.appendChild(this.element);
        }
        this.callback = g;
        var k = this;
        this.push = function() {
        ...
        this.clear = function() {
            if (k.pushed) {
                var a = 1e3 / f
                  , b = 1;
                "undefined" != typeof k.interval && clearInterval(k.interval),
                k.interval = setInterval(function() {
                    b -= 1 / (e / a),
                    k.element.style.opacity = b,
                    0 >= b && (clearInterval(k.interval),
                    k.destroy())
                }, a)
            }
        }
    ...
    return function Notifier(c) {
        ...
        this.notify = function(a, c, d, e) {
            if ("undefined" == typeof this.options[a])
                return void console.error("Notify.js: Error, undefined '" + a + "' notification type");
            "undefined" == typeof d && (d = this.options.default_time);
            var f = new b(this,c,this.options[a],d,this.options.vanish_time,this.options.fps,e);
            return this.options.autopush && f.push(),
            f
        }
    }
});
